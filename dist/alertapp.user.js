...
`,A=!1;function j(n){A&&(document.getElementById("aa-root")?.remove(),document.getElementById("aa-style")?.remove(),A=!1),A=!0;let e=document.createElement("style");e.id="aa-style",e.textContent=ae,document.head.appendChild(e);let t=document.createElement("div");t.id="aa-root",document.body.appendChild(t);let r=document.createElement("button");r.className="aa-btn",r.setAttribute("aria-label","\uC54C\uB78C \uC704\uC82F \uC5F4\uAE30"),r.innerHTML='<span style="position:relative;display:inline-flex;align-items:center;justify-content:center">\u{1F514}<span class="aa-count" id="aa-count" style="display:none">0</span></span>',t.appendChild(r);let a=document.createElement("div");a.className="aa-panel",a.id="aa-panel",a.innerHTML=`
  ...
  `,t.appendChild(o);let s=null,c=!0;function u(i){let l=document.getElementById("aa-count");l&&(i>0?(l.textContent=String(i),l.style.display="block"):l.style.display="none")}function p(i){let l=document.getElementById("aa-alert-list");if(l){if(l.innerHTML="",i.length===0){l.innerHTML='<div class="aa-empty">\uC54C\uB78C \uC5C6\uC74C</div>';return}i.forEach(d=>{let g=document.createElement("div");g.className="aa-alert-item";let C=d.direction==="cross_above"?"\u2191 \uC704\uB85C cross":"\u2193 \uC544\uB798\uB85C cross",X=d.status==="armed"?"aa-badge-armed":d.status==="triggered"?"aa-badge-triggered":"aa-badge-paused",q=d.status==="armed"?"\uB300\uAE30":d.status==="triggered"?"\uBC1C\uD654":"\uC815\uC9C0";g.innerHTML=`
      ...
      `,g.querySelector(".aa-del-btn")?.addEventListener("click",()=>{n.onRemoveAlert(d.id).then(()=>b()).catch(console.warn)}),l.appendChild(g)})}}async function v(){try{let i=await n.getTelegramConfig(),l=document.getElementById("aa-tg-warn-icon");if(!i)l&&(l.textContent=" \u26A0\uFE0F");else{l&&(l.textContent="");let d=document.getElementById("aa-tg-token"),g=document.getElementById("aa-tg-chat");d&&(d.placeholder="\uC800\uC7A5\uB428 (\uBCC0\uACBD \uC2DC \uC785\uB825)"),g&&(g.value=i.chatId)}}catch(i){console.warn("alertapp: Telegram \uC124\uC815 \uB85C\uB4DC \uC2E4\uD328",i)}}async function b(){try{let i=await n.getAlerts();p(i),u(i.filter(l=>l.status==="armed").length),await v()}catch(i){console.warn("alertapp: refresh \uC2E4\uD328",i)}}r.addEventListener("click",()=>{a.classList.toggle("aa-open"),a.classList.contains("aa-open")&&b().catch(console.warn)}),document.getElementById("aa-tg-toggle")?.addEventListener("click",()=>{c=!c;let i=document.getElementById("aa-tg-body"),l=document.getElementById("aa-tg-arrow");i&&i.classList.toggle("aa-open",!c),l&&(l.textContent=c?"\u25B6":"\u25BC")}),document.getElementById("aa-tg-save")?.addEventListener("click",()=>{let i=document.getElementById("aa-tg-token"),l=document.getElementById("aa-tg-chat"),d=i?.value?.trim()??"",g=l?.value?.trim()??"";if(!d||!g){alert("Bot Token \uACFC Chat ID \uB97C \uBAA8\uB450 \uC785\uB825\uD574\uC8FC\uC138\uC694.");return}n.onSetTelegramConfig(d,g).then(()=>{i&&(i.value="",i.placeholder="\uC800\uC7A5\uB428 (\uBCC0\uACBD \uC2DC \uC785\uB825)"),v().catch(console.warn)}).catch(C=>{console.warn("alertapp: Telegram \uC800\uC7A5 \uC2E4\uD328",C),alert("\uC800\uC7A5 \uC2E4\uD328. \uCF58\uC194\uC744 \uD655\uC778\uD574\uC8FC\uC138\uC694.")})}),document.getElementById("aa-add-btn")?.addEventListener("click",()=>{try{let i=k();if(i.length===0){alert("\uBA3C\uC800 \uCD94\uC138\uC120\uC744 \uADF8\uB824\uC8FC\uC138\uC694.");return}let l=i[i.length-1];i.length>1&&alert(`\uCD94\uC138\uC120\uC774 ${i.length}\uAC1C \uAC10\uC9C0\uB410\uC2B5\uB2C8\uB2E4. \uAC00\uC7A5 \uCD5C\uADFC \uADF8\uB9B0 \uCD94\uC138\uC120\uC744 \uC0AC\uC6A9\uD569\uB2C8\uB2E4.`),s=l.id;let d=document.getElementById("aa-dir-dialog");if(d){d.classList.add("aa-open");let g=a.getBoundingClientRect();d.style.top=`${g.bottom+8}px`,d.style.right="16px"}}catch(i){console.warn("alertapp: Alert \uCD94\uAC00 \uC2E4\uD328",i)}});function E(){document.getElementById("aa-dir-dialog")?.classList.remove("aa-open"),s=null}return document.getElementById("aa-dir-above")?.addEventListener("click",()=>{E(),n.onAddAlert("cross_above").then(()=>b()).catch(console.warn)}),document.getElementById("aa-dir-below")?.addEventListener("click",()=>{E(),n.onAddAlert("cross_below").then(()=>b()).catch(console.warn)}),document.getElementById("aa-dir-cancel")?.addEventListener("click",E),b().catch(console.warn),{destroy(){t.remove(),e.remove(),A=!1},async refresh(){await b()}}}function B(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function L(n,e,t){let r=e.time-n.time;if(r===0)return NaN;let a=(e.price-n.price)/r;return n.price+a*(t-n.time)}function V(n,e,t){if(n.symbol!==t.symbol)return{triggered:!1,linePrice:NaN,reason:"wrong_symbol"};if(n.status!=="armed")return{triggered:!1,linePrice:L(n.p1,n.p2,t.ts),reason:"paused"};if(e===null)return{triggered:!1,linePrice:L(n.p1,n.p2,t.ts),reason:"no_change"};let r=L(n.p1,n.p2,t.ts);if(isNaN(r))return{triggered:!1,linePrice:NaN,reason:"no_change"};let a=e.price,o=t.price;if(n.direction==="cross_above"){if(a<r&&o>=r)return{triggered:!0,linePrice:r,reason:"cross_above"}}else if(a>r&&o<=r)return{triggered:!0,linePrice:r,reason:"cross_below"};return{triggered:!1,linePrice:r,reason:"no_change"}}var oe="https://api.telegram.org";var K="...";function ie(n){return n.length<=4096?n:n.slice(0,4096-K.length)+K}function se(n,e,t){let r={chat_id:n.chatId,text:ie(e)};return t?.parseMode&&t.parseMode!=="plain"&&(r.parse_mode=t.parseMode),r}function le(n,e){return new Promise(t=>{GM_xmlhttpRequest({method:"POST",url:n,headers:{"Content-Type":"application/json"},data:e,onload(r){try{let a=r.status;a>=200&&a<300?t({ok:!0,status:a}):t({ok:!1,status:a,error:`http_${a}`})}catch{t({ok:!1,error:"gm_parse_error"})}},onerror(r){t({ok:!1,status:r.status,error:"gm_network_error"})}})})}async function de(n,e){let r=(await globalThis.fetch(n,{method:"POST",headers:{"Content-Type":"application/json"},body:e})).status;return r>=200&&r<300?{ok:!0,status:r}:{ok:!1,status:r,error:`http_${r}`}}async function J(n,e,t){if(!n.botToken)return{ok:!1,error:"missing_token"};if(!n.chatId)return{ok:!1,error:"missing_chat_id"};let r=`${oe}/bot${n.botToken}/sendMessage`,a=JSON.stringify(se(n,e,t));try{return typeof GM_xmlhttpRequest<"u"?await le(r,a):await de(r,a)}catch(o){return{ok:!1,error:`network_error: ${o instanceof Error?o.message:String(o)}`}}}var S=class{_ws;_onTrigger;_prevTick=new Map;_handlers=new Map;_tickCount=new Map;constructor(e){this._ws=e.ws,this._onTrigger=e.onTrigger}async start(){let e=await m();for(let t of e.alerts)t.status==="armed"&&this.subscribe(t);console.info("[alertapp:runner] start: armed alerts subscribed =",e.alerts.filter(t=>t.status==="armed").length)}subscribe(e){if(this._handlers.has(e.id))return;console.info("[alertapp:runner] subscribe alert",{id:e.id,symbol:e.symbol,direction:e.direction,p1:e.p1,p2:e.p2,status:e.status});let t=r=>{this._onTick(e,r)};this._handlers.set(e.id,t),this._ws.subscribe(e.symbol,t)}unsubscribe(e){let t=this._handlers.get(e.id);t&&(this._handlers.delete(e.id),this._ws.unsubscribe(e.symbol,t))}stop(){for(let[e]of this._handlers){let t=this._handlers.get(e)}this._handlers.clear(),this._prevTick.clear(),this._ws.close()}_onTick(e,t){let r=(this._tickCount.get(t.symbol)??0)+1;this._tickCount.set(t.symbol,r),(r===1||r%30===0)&&console.info("[alertapp:runner] tick",{symbol:t.symbol,price:t.price,ts:t.ts,cnt:r});let a=this._prevTick.get(t.symbol)??null,o=V(e,a,t);this._prevTick.set(t.symbol,t),o.reason!=="no_change"&&console.info("[alertapp:runner] evaluateAlert",{alertId:e.id,direction:e.direction,linePrice:o.linePrice,reason:o.reason,triggered:o.triggered,prevPrice:a?.price??null,currPrice:t.price}),o.triggered&&(console.info("[alertapp:runner] \u25B2 TRIGGERED",e.id,"sending telegram..."),this._handleTrigger(e,t).catch(()=>{}))}async _handleTrigger(e,t){let r=Date.now();await D(e.id,{status:"triggered",triggeredAt:r}),this.unsubscribe(e);let a=await w();if(a){let s=e.direction==="cross_above"?"\uC704\uB85C cross":"\uC544\uB798\uB85C cross",c=t.price.toLocaleString(),u=`[alertapp] ${e.symbol} \uC54C\uB78C \uBC1C\uD654
\uBC29\uD5A5: ${s}
\uBC1C\uD654 \uAC00\uACA9: ${c}
\uC2DC\uAC01: ${new Date(r).toLocaleString("ko-KR")}`,p=await J(a,u);console.info("[alertapp:runner] telegram send",{ok:p.ok,status:p.status,error:p.error})}else console.warn("[alertapp:runner] telegram config \uBBF8\uC124\uC815 \u2014 alert triggered \uD558\uC9C0\uB9CC \uBA54\uC2DC\uC9C0 \uBBF8\uC804\uC1A1. \uC704\uC82F\uC758 Telegram \uC124\uC815 \uB4F1\uB85D \uD544\uC694.");let o={...e,status:"triggered",triggeredAt:r};this._onTrigger?.(o,t)}};function ce(){return typeof GM_setValue=="function"&&typeof GM_getValue=="function"&&typeof GM_xmlhttpRequest=="function"}var h=null,f=null,R=null;function pe(){h?.stop(),f?.destroy(),R?.(),h=null,f=null,R=null}async function ue(){if(console.info("[alertapp] main() entry"),!ce()){console.warn("[alertapp] Tampermonkey GM API \uBBF8\uC874\uC7AC. \uC2A4\uD06C\uB9BD\uD2B8\uB97C \uC885\uB8CC\uD569\uB2C8\uB2E4.");return}console.info("[alertapp] assertTampermonkey OK");try{await F(),console.info("[alertapp] waitForTvChart resolved")}catch(e){console.warn("[alertapp] waitForTvChart rejected",e),alert("alertapp: TradingView \uCC28\uD2B8 \uB85C\uB4DC\uB97C \uAE30\uB2E4\uB9AC\uB2E4 \uC2DC\uAC04 \uCD08\uACFC\uB410\uC2B5\uB2C8\uB2E4. \uD398\uC774\uC9C0\uB97C \uC0C8\uB85C\uACE0\uCE68\uD574 \uC8FC\uC138\uC694.");return}let n=z();h=new S({ws:n,onTrigger:(e,t)=>{f?.refresh().catch(console.warn)}}),await h.start(),console.info("[alertapp] AlertRunner started"),console.info("[alertapp] mountWidget call"),f=j({async onAddAlert(e){let t=k();if(t.length===0){alert("\uBA3C\uC800 \uCD94\uC138\uC120\uC744 \uADF8\uB824\uC8FC\uC138\uC694.");return}let r=t[t.length-1],a=y();if(!a.binanceSymbol){alert(`alertapp: Binance Spot \uC2EC\uBCFC\uC744 \uC778\uC2DD\uD558\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.
BINANCE:BTCUSDT \uD615\uC2DD\uC758 \uCC28\uD2B8\uC5D0\uC11C \uC0AC\uC6A9\uD574\uC8FC\uC138\uC694.`);return}let o={id:crypto.randomUUID(),symbol:a.binanceSymbol,exchange:"binance",tfLabel:"\u2013",p1:r.p1,p2:r.p2,direction:e,status:"armed",createdAt:Date.now(),triggeredAt:null};await N(o),h?.subscribe(o),console.info("[alertapp] alert added:",o.id,o.direction,o.symbol),await f?.refresh()},async onRemoveAlert(e){let r=(await m()).alerts.find(a=>a.id===e);r&&h?.unsubscribe(r),await G(e)},async onSetTelegramConfig(e,t){await H({botToken:e,chatId:t}),console.info("alertapp: Telegram \uC124\uC815\uC774 \uC800\uC7A5\uB410\uC2B5\uB2C8\uB2E4.")},async getAlerts(){return(await m()).alerts},async getTelegramConfig(){return w()}}),console.info("[alertapp] mountWidget done",f?"OK":"NULL"),R=U(e=>{f?.refresh().catch(console.warn)}),console.info("[alertapp] subscribeSymbolChange wired"),window.addEventListener("beforeunload",pe),console.info("[alertapp] main() complete")}ue().then(()=>console.info("[alertapp] main() resolved")).catch(n=>{console.error("[alertapp] main() \uC624\uB958",n)});})();
